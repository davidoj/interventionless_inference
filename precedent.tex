...
Suppose we have an absolutely continuous structural decision model $(\prob{P}_\cdot,\Omega,C)$ where we have observed $\RV{Z}_i$ independent of $\RV{Y}_i$ given $\RV{X}_i$, and for which we have an arbitrary graph $\graph{G}$ satisfying the following three assumptions:
\begin{enumerate}
